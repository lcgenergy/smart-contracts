...
contract LCG is ERC20, Ownable {
  string public constant name = "LCG Energy";
  string public constant symbol = "LCG";
  uint32 public constant decimals = 18;

  uint256 public totalAmount = uint256(2e9).mul(1 ether);
  uint256 public saleAmount = uint256(14e8).mul(1 ether);
  ...
  */
  constructor(address newOwner) public {
    ...
    _mint(newOwner, totalAmount.sub(saleAmount));
  }
  ...
  function activateSaleContract(address crowdsale) public onlyOwner {
    ...
    _mint(crowdsaleAddress, saleAmount);
    emit CrowdsaleActivation(crowdsaleAddress, saleAmount);
  }

  function burnUnsoldTokens() public returns(bool) {
    require(msg.sender == crowdsaleAddress);
    uint256 balance = balanceOf(msg.sender);
    if (balance > 0) _burn(msg.sender, balance);
    return true;
  }
}